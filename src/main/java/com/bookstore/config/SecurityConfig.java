...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http.formLogin()
                .loginPage("/account/sign-in") // 로그인 페이지 URL 설정
                ...
                .failureUrl("/account/sign-in/error") // 로그인 실패 시 이동할 URL 설정
                .and()
                .logout() // 로그아웃 처리
        ...
        http.rememberMe()
                ...
                .tokenRepository(tokenRepository()) // DB 에서 토큰 값 가져오기 (Username, 토큰, 시리즈)
        ;


        http.authorizeRequests() // 시큐리티 처리에 HttpServletRequest 이용
                .mvcMatchers("/", "/login", "/logout", "/account/**", "/bookSearchDetail", "/search").permitAll()
                // 메인, 회원 관련, 상품 관련, 상품 이미지 관련 페이지는 모든 사용자가 로그인(인증)없이 접근 가능
                .mvcMatchers("/admin/**").hasRole("ADMIN") // admin 으로 시작하는 경로는 계정이 ADMIN 일 경우에만 접근 가능
                .anyRequest().authenticated() // 위 mvcMatchers 로 설정해준 결로 외 나머지 경로들은 모두 인증 요구
        ;
    ...
    public void configure(WebSecurity web) throws Exception {
        web.ignoring().antMatchers("/css/**", "/img/**", "/js/**", "/mapper/**", "/error", "/image/**");
